"""Runtime smoke harness for business correction guard hotfix 8.5.22.1."""
...
def main() -> int:
    ...
    if APP_VERSION != "3.3.5.5.8.5.22.1":
    ...
    if not STOP_PATH_MINIMAL_MODE or not RUNTIME_EVIDENCE_PACKAGE_DISABLED:
        ...
        return 1
    if not AUTO_EXPORT_ALPHA_TXT_ENABLED or not FINAL_UI_UPDATE_TIMED_OUT_KWARG_FIX_ENABLED:
        print("FAILED hotfix flags missing")
        return 1
    if TEMP_AUDIO_RETENTION_HOURS != 2:
        print(f"FAILED retention hours {TEMP_AUDIO_RETENTION_HOURS}")
        return 1

    selftest = run_business_correction_guard_selftest()
    if not selftest.get("ok"):
        print(f"FAILED guard selftest {selftest.get('failures')}")
        return 1
    ...
    print(f"PASSED elapsed={elapsed:.2f}s alpha_txt={alpha_txt.exists()}")
