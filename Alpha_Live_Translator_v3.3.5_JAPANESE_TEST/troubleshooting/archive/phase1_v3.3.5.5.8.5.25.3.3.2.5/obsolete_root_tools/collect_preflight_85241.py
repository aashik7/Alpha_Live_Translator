"""Preflight evidence collection before 8.5.24.1 changes."""
...
def main() -> int:
    ...
    preflight = root / "preflight_85241"
    preflight.mkdir(parents=True, exist_ok=True)
    print("PREFLIGHT_85241_COLLECTION_STARTED")
    warnings: list[str] = []
    ...
    files: list[tuple[Path, str]] = [
        ...
        (root / "runs" / "_pending" / "accuracy" / "boundary_stabilizer_summary.json", "boundary_stabilizer_summary_pending.json"),
        (root / "Cursor final report.txt", "Cursor_final_report.txt"),
        (root / "validation" / "validate_accuracy_8524_output.txt", "validate_accuracy_8524_output.txt"),
    ]
    for smoke in sorted((root / "smoke_tests").glob("*8524*")) if (root / "smoke_tests").exists() else []:
    ...
    if runs:
        acc = runs[0] / "accuracy"
        for name in ("boundary_stabilizer_summary.json", "boundary_stabilizer_decisions.jsonl"):
            p = acc / name
            if p.exists():
                files.append((p, name))
    ...
    for src, dst in files:
        if src.exists():
            ...
            print(f"PREFLIGHT_85241_FILE_COLLECTED: {src}")
        else:
            print(f"PREFLIGHT_85241_FILE_MISSING: {src}")
            if dst in critical:
    ...
    manifest = {
        ...
        "app_version_before_change": "3.3.5.5.8.5.24",
        ...
        "boundary_summary_path": boundary_summary,
        "boundary_decisions_path": boundary_decisions,
        "warnings": warnings,
    }
    (preflight / "PREFLIGHT_85241_MANIFEST.json").write_text(
        json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (preflight / "PREFLIGHT_85241_SUMMARY.txt").write_text(
        "\n".join(
            [
                "PREFLIGHT 85241 SUMMARY",
                ...
                f"boundary_summary_path={boundary_summary or 'none'}",
                f"warnings={warnings}",
            ]
        )
        + "\n",
    ...
    print("PREFLIGHT_85241_MANIFEST_WRITTEN")
    print("PREFLIGHT_85241_COLLECTION_COMPLETED")
