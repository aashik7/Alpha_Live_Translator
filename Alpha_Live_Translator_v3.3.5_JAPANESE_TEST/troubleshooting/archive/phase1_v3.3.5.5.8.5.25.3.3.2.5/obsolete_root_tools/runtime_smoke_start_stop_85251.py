"""Runtime smoke harness for Lossless Clean Export 8.5.25.1."""
...
def main() -> int:
    ...
    if APP_VERSION != "3.3.5.5.8.5.25.1":
    ...
    out_lines, cov, _ = finalize_lossless_clean_export(["テスト用の売上高は百三十六億円となりました。"])
    if not out_lines:
        print("FAILED lossless export gate returned empty")
        return 1

    before = {str(p): _file_hash(p) for p in _protected_paths()}
    set_alpha_export_run_type(RUN_TYPE_SMOKE_TEST)
    write_smoke_test_alpha_outputs("smoke lossless export test", run_type=RUN_TYPE_SMOKE_TEST)
    host = _DummyHost()
    t0 = time.monotonic()
    ...
    smoke_out = smoke_dir / f"runtime_smoke_start_stop_85251_{ts}.txt"
    ...
    print(f"PASSED elapsed={elapsed:.2f}s lossless_gate_not_blocking_stop=True")
    return 0
