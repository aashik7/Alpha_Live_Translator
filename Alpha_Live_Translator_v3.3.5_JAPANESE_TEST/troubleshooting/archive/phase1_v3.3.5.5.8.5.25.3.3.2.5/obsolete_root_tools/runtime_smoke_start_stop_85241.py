"""Runtime smoke harness for Boundary Merge Revision 8.5.24.1."""
...
def main() -> int:
    ...
    if APP_VERSION != "3.3.5.5.8.5.24.1":
    ...
    smoke_out = smoke_dir / f"runtime_smoke_start_stop_85241_{ts}.txt"
    smoke_out.write_text(
        f"PASSED elapsed={elapsed:.2f}s live_alpha_protected={unchanged} "
        f"smoke_pointer={smoke_pointer.exists()} live_pointer_unchanged={live_pointer_before == live_pointer_after}\n",
        encoding="utf-8",
    )
    ...
    print(
        ...
        f"live_alpha_protected={unchanged} revision_model_not_blocking_stop=True"
    )
    return 0
