"""Runtime smoke harness for evidence protection 8.5.23.2."""
...
def main() -> int:
    ...
    if APP_VERSION != "3.3.5.5.8.5.23.2":
    ...
    before_sizes = {str(p): p.stat().st_size if p.exists() else 0 for p in _protected_paths()}
    ...
    if not unchanged:
        print(f"FAILED live alpha overwritten before={before_sizes} after_changed=True")
        return 1
    if not smoke_written:
        print("FAILED smoke test output not written to smoke_tests folder")
        return 1

    print(
        f"PASSED elapsed={elapsed:.2f}s smoke_written={smoke_written} "
        f"live_alpha_protected={unchanged}"
    )
