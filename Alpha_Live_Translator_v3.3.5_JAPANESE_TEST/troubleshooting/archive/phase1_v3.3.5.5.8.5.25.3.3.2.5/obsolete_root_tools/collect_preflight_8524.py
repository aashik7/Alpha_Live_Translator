"""Preflight evidence collection before 8.5.24 changes."""
...
def main() -> int:
    ...
    preflight = root / "preflight_8524"
    preflight.mkdir(parents=True, exist_ok=True)
    print("PREFLIGHT_8524_COLLECTION_STARTED")
    warnings: list[str] = []
    ...
    files: list[tuple[Path, str]] = [
        ...
        (
            root / "accuracy_benchmark" / "reference_transcripts" / "business_18min_test01.txt",
            "business_18min_test01.txt",
        ),
        (root / "accuracy_benchmark" / "reference_transcripts" / "test01.txt", "test01_reference.txt"),
        (root / "Cursor final report.txt", "Cursor_final_report.txt"),
        (root / "validation" / "validate_accuracy_852341_output.txt", "validate_accuracy_852341_output.txt"),
    ]
    for smoke in sorted((root / "smoke_tests").glob("*852341*")) if (root / "smoke_tests").exists() else []:
    ...
    for src, dst in files:
        if src.exists():
            ...
            print(f"PREFLIGHT_8524_FILE_COLLECTED: {src}")
        else:
            print(f"PREFLIGHT_8524_FILE_MISSING: {src}")
            if dst in critical:
    ...
    manifest = {
        ...
        "app_version_before_change": "3.3.5.5.8.5.23.4.1",
    ...
    (preflight / "PREFLIGHT_8524_MANIFEST.json").write_text(
        json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (preflight / "PREFLIGHT_8524_SUMMARY.txt").write_text(
        "\n".join(
            [
                "PREFLIGHT 8524 SUMMARY",
    ...
    print("PREFLIGHT_8524_MANIFEST_WRITTEN")
    print("PREFLIGHT_8524_COLLECTION_COMPLETED")
