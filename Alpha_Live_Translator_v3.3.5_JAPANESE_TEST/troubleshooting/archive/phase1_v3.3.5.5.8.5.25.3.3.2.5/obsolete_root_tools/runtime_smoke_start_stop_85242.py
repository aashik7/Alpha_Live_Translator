"""Runtime smoke harness for Residual Duplicate & Punctuation Cleanup 8.5.24.2."""
...
def main() -> int:
    ...
    if APP_VERSION != "3.3.5.5.8.5.24.2":
    ...
    out, metrics = sweep_residual_duplicates(["短い。", "短い。長い続き。"])
    if len(out) != 1:
        print(f"FAILED cleanup sweep blocked unexpectedly out={out}")
        return 1
    ...
    write_smoke_test_alpha_outputs("smoke test line", run_type=RUN_TYPE_SMOKE_TEST)
    host = _DummyHost()
    t0 = time.monotonic()
    ...
    live_pointer_after = _file_hash(Path("troubleshooting/latest/LATEST_LIVE_RUN_POINTER.json"))
    ...
    smoke_out = smoke_dir / f"runtime_smoke_start_stop_85242_{ts}.txt"
        ...
        json.dumps(
            {
                ...
                "smoke_pointer_written": smoke_pointer.exists(),
                "live_pointer_unchanged": live_pointer_before == live_pointer_after,
                "cleanup_sweep_ok": True,
                "residual_duplicate_after_count": metrics.get("residual_duplicate_after_count", 0),
            },
            ensure_ascii=False,
            indent=2,
        )
    ...
    if not unchanged:
        ...
        return 1
    if live_pointer_before and live_pointer_before != live_pointer_after:
        print("FAILED smoke run overwrote LATEST_LIVE_RUN_POINTER")
        return 1
    ...
    print(
        f"PASSED elapsed={elapsed:.2f}s smoke_written=True "
        f"live_alpha_protected={unchanged} cleanup_not_blocking_stop=True"
    )
    return 0
