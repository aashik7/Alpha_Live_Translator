...
def write_final_alpha_output_from_snapshot(host: Any = None) -> Optional[Path]:
    ...
    if not snap:
        unverified = ""
        try:
            unverified = get_transcript_text_from_snapshot()
        except Exception:
            pass
        if not unverified.strip() and host is not None:
            try:
                unverified = get_transcript_text_from_host(host, allow_ui_export=False)
            except Exception:
                pass
        try:
            emergency_path.parent.mkdir(parents=True, exist_ok=True)
            emergency_path.write_text(unverified or "", encoding="utf-8")
            from alpha.utils.japanese_accuracy_log import jp_accuracy_log

            jp_accuracy_log(
                "EMERGENCY_UNVERIFIED_TRANSCRIPT_WRITTEN",
                path=str(emergency_path),
                reason="no_frozen_snapshot",
            )
        except Exception:
            pass
