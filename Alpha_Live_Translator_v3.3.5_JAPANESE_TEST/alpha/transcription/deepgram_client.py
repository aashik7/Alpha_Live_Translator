...
class DeepgramClientMixin:
    ...
    def _deepgram_worker(self):
            ...
            stream_opts = self._resolve_deepgram_stream_options(
                FORCE_DEEPGRAM_LANGUAGE or lang_code or "ja"
            )
            stt_profile = self._resolve_japanese_stt_profile(
                FORCE_DEEPGRAM_LANGUAGE or lang_code or "ja"
            )
            endpointing_ms = stream_opts["endpointing_ms"]
            utterance_end_ms = stream_opts["utterance_end_ms"]
            ...
            try:
                ...
                payload = build_deepgram_request_actual_payload(
                    ...
                    model=str(DEEPGRAM_MODEL),
                    language=str(getattr(self, "_last_deepgram_language", lang_code) or lang_code),
                    encoding="linear16",
                    sample_rate=int(
                ...
                try:
                    ...
                    pass
                write_deepgram_request_actual(payload)
            except Exception:
                pass
            try:
