...
# Category-match-only normalizer version (does not affect strict CER).
CATEGORY_MATCH_NORMALIZER_VERSION = "category_match_v26.5.2"
...
def normalize_text(text: str) -> str:
    """Strict CER normalizer — whitespace/speaker-tag only. Do not alter."""
    return normalize_transcript_text(text)


_LATIN_LETTER_WS_RE = re.compile(r"(?<=[A-Za-z])\s+(?=[A-Za-z])")
_LATIN_ACRONYM_PUNCT_RE = re.compile(r"(?<=[A-Za-z])[/／･・\u30FB\-‐‑–—―_](?=[A-Za-z])")
_CATEGORY_WS_RE = re.compile(r"\s+")


def normalize_category_match_text(text: str) -> str:
    """Generic category-match normalizer (analysis-only; never used for strict CER).

    Applies only:
    - Unicode NFKC
    - ASCII casefolding (via str.casefold)
    - removal of whitespace between split Latin letters
    - punctuation-equivalent handling for Latin acronym forms (A/B ↔ AB)

    Does not add fuzzy Japanese matching, kanji equivalence, name aliases,
    numeric value equivalence, or benchmark-specific pairs.
    """
    out = unicodedata.normalize("NFKC", text or "")
    # Collapse whitespace between Latin letters before full whitespace strip so
    # forms such as "C R M" / "c rm" become contiguous prior to casefold compare.
    out = _LATIN_LETTER_WS_RE.sub("", out)
    out = out.casefold()
    out = _CATEGORY_WS_RE.sub("", out)
    # A/B ↔ AB (and fullwidth / middle-dot variants between Latin letters only).
    out = _LATIN_ACRONYM_PUNCT_RE.sub("", out)
    return out
...
def _term_in_hyp(term: str, hyp_norm: str) -> bool:
    """Category-entity presence check (truthful generic matching; not strict CER)."""
    term_norm = normalize_text(term)
    if not term_norm:
        return False
    term_cat = normalize_category_match_text(term_norm)
    hyp_cat = normalize_category_match_text(hyp_norm)
    if term_cat and term_cat in hyp_cat:
        return True
    canon, _ = apply_meaning_equivalent(term_norm)
    hyp_canon, _ = apply_meaning_equivalent(hyp_norm)
    canon_cat = normalize_category_match_text(canon)
    hyp_canon_cat = normalize_category_match_text(hyp_canon)
    return bool(canon_cat) and canon_cat in hyp_canon_cat
...
def score_domain_categories(
    *,
    ...
    return {
        ...
        "category_match_normalizer_version": CATEGORY_MATCH_NORMALIZER_VERSION,
        ...
        "combined_business_term_found_count": int(business_found),
        "combined_business_term_expected_count": int(business_expected),
...
def score_all(
    ...
    *,
    ...
    # Identical semantic boundary for Raw/Stable/Final. Boundary-spanning Stable/Final
    # records keep their in-window text (do not discard the whole record).
    raw_text = clip_text_to_semantic_window(raw_text, window) or raw_text
    stable_text = clip_text_to_semantic_window(stable_text_full, window)
    final_text = clip_text_to_semantic_window(final_text_full, window)
