...
class SilenceRunsAreCollapsedTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.atc._silence_runs.clear()



    def _packets(self):

        self.atc._flush_all_silence_runs()
    ...
    def _append(self, key, classification, frames):
        ...
        if classification == SILENCE:
            ...
            run = self.atc._silence_runs.get(key)

            if run is None:

                self.atc._silence_runs[key] = {
            ...
            self.atc._flush_silence_run_locked(key)
...
class RealIngestPathTest(unittest.TestCase):
    ...
    def test_real_active_packets_are_not_collapsed(self):
