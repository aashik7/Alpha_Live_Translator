...
def _numpy():
    """Lazy numpy — avoids pulling NumPy during UI import / first paint."""
    global _np
    if _np is None:
        import numpy as np
...
# Baseline profiling only: force NumPy import at module load.
import os as _os

if _os.environ.get("ALPHA_STARTUP_EAGER_NUMPY", "").strip().lower() in (
    "1",
    "true",
    "yes",
):
