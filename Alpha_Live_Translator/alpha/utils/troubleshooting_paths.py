...
def rebind_all_runtime_writers(run_folder: Path, *, startup_phase: bool = False) -> None:
    ...
    if STARTUP_RECOVERY_MODE and startup_phase and _is_ui_thread():
        ...
        return
    global _writers_rebound, _run_rebind_count
    try:
        _run_rebind_count = int(globals().get("_run_rebind_count", 0) or 0) + 1
    except Exception:
        _run_rebind_count = 1
    if int(_run_rebind_count) > 1:
        try:
            from alpha.utils.japanese_accuracy_log import jp_accuracy_log
    ...
    try:
        ...
        jp_accuracy_log(
            "ALL_WRITERS_BOUND_TO_RUN",
            run_folder=str(run_folder),
            run_rebind_count=int(globals().get("_run_rebind_count", 1) or 1),
            pending_writers_remaining=len(pending_after or []),
        )
        jp_accuracy_log(
            "RUN_REBIND_COUNT",
            run_rebind_count=int(globals().get("_run_rebind_count", 1) or 1),
...
def _deepgram_snapshot_for_selected_language(selected_language: str) -> dict[str, Any]:
    """Language-accurate Deepgram snapshot for RUN_MANIFEST (not JA-constant bias)."""
    from alpha.stt_settings import (
        DEEPGRAM_ENDPOINTING_MS as EN_ENDPOINTING_MS,
        DEEPGRAM_JA_ENDPOINTING_MS,
        DEEPGRAM_JA_UTTERANCE_END_MS,
        DEEPGRAM_MODEL as STT_MODEL,
        DEEPGRAM_UTTERANCE_END_MS as EN_UTTERANCE_END_MS,
        clamp_deepgram_utterance_end_ms,
    )

    lang = str(selected_language or "").strip().lower()
    if lang.startswith("ja"):
        endpointing = int(DEEPGRAM_JA_ENDPOINTING_MS)
        utterance_end, _ = clamp_deepgram_utterance_end_ms(int(DEEPGRAM_JA_UTTERANCE_END_MS))
        code = "ja"
        diarize = "absent"
    elif lang.startswith("en"):
        endpointing = int(EN_ENDPOINTING_MS)
        utterance_end, _ = clamp_deepgram_utterance_end_ms(int(EN_UTTERANCE_END_MS))
        code = "en"
        try:
            from alpha.constants import ENGLISH_DIARIZATION_ENABLED

            diarize = "latest" if ENGLISH_DIARIZATION_ENABLED else "absent"
        except Exception:
            diarize = "absent"
    else:
        endpointing = int(EN_ENDPOINTING_MS)
        utterance_end, _ = clamp_deepgram_utterance_end_ms(int(EN_UTTERANCE_END_MS))
        code = lang or "unknown"
        diarize = "absent"
    return {
        "model": STT_MODEL or DEEPGRAM_MODEL,
        "language": code,
...
def update_run_manifest_deepgram_actual(actual: dict[str, Any]) -> None:
    """Bind RUN_MANIFEST Deepgram snapshot to the actual request after connect."""
    global _run_manifest
    if not isinstance(_run_manifest, dict):
        return
    lang = str(
        actual.get("language")
        or actual.get("actual_deepgram_language")
        or _run_manifest.get("selected_language")
        or ""
    ).lower()
    snap = {
        "model": str(actual.get("model") or DEEPGRAM_MODEL),
        "language": lang[:2] if lang else str(_run_manifest.get("selected_language") or ""),
        "endpointing_ms": int(actual.get("endpointing") or actual.get("endpointing_ms") or 0),
        "utterance_end_ms": int(
            actual.get("utterance_end_ms") or actual.get("utterance_end") or 0
        ),
        "diarize_model": str(
            actual.get("diarize_model")
            or ("latest" if actual.get("diarize") else "absent")
        ),
    }
    _run_manifest["deepgram_config_snapshot"] = snap
    _run_manifest["actual_deepgram_language"] = snap["language"]
    try:
        path = Path(_run_manifest.get("troubleshooting_folder") or "") / "RUN_MANIFEST.json"
...
def create_run_folder(
    *,
    ...
    manifest = {
        ...
        "deepgram_config_snapshot": _deepgram_snapshot_for_selected_language(
            selected_language
        ),
        "actual_deepgram_language": str(selected_language or "").strip().lower()[:8],
        "japanese_profile": {
...
def reset_troubleshooting_session() -> None:
    global _current_run_folder, _run_manifest, _writers_rebound, _run_folder_created_at, _active_run_id, _run_rebind_count
    with _lock:
