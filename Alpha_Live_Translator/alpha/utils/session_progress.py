...
_UI_PERF_METRICS_INTERVAL_S = 60.0
_last_ui_perf_metrics_mono: float = 0.0
...
def maybe_emit_long_session_health_heartbeat(host: Any = None, *, force: bool = False) -> None:
    global _last_long_session_health_mono, _last_health_timeline_mono, _last_ui_perf_metrics_mono
    ...
    # Phase 5: UI performance metrics every 60 seconds.
    if force or (now - _last_ui_perf_metrics_mono) >= _UI_PERF_METRICS_INTERVAL_S:
        _last_ui_perf_metrics_mono = now
        try:
            emit_ui_performance_metrics(host)
        except Exception:
            pass


def emit_ui_performance_metrics(host: Any = None) -> dict[str, Any]:
    """Record bounded UI/transcript/logger performance metrics."""
    from alpha.utils.japanese_accuracy_log import jp_accuracy_log

    hist = list(getattr(host, "_ui_insert_durations_ms", []) or []) if host else []
    avg_insert = (sum(hist) / len(hist)) if hist else 0.0
    max_insert = max(hist) if hist else 0.0
    async_health: dict[str, Any] = {}
    try:
        from alpha.utils.async_debug_log import get_async_log_health
    ...
    canonical_count = 0
    try:
        store = getattr(host, "transcript_store", None) if host else None
        if store is not None and hasattr(store, "segment_count"):
            canonical_count = int(store.segment_count())
    except Exception:
        pass
    payload = {
        "runtime_seconds": elapsed_listening_seconds(),
        "process_memory_rss_mb": rss_mb,
        "canonical_segment_count": canonical_count,
        "rendered_ui_segment_count": int(getattr(host, "_displayed_segment_count", 0) or 0) if host else 0,
        "transcript_ui_queue_size": _safe_qsize(
            getattr(host, "transcript_queue", None) if host else None
        ),
        "audio_queue_size": _safe_qsize(getattr(host, "_audio_q", None) if host else None),
        "ui_event_posted_count": int(getattr(host, "_transcript_events_posted", 0) or 0) if host else 0,
        "ui_event_drained_count": int(getattr(host, "_transcript_events_drained", 0) or 0) if host else 0,
        "ui_event_dropped_count": int(getattr(host, "_transcript_events_dropped", 0) or 0) if host else 0,
        "average_ui_insert_ms": round(avg_insert, 3),
        "maximum_ui_insert_ms": round(max_insert, 3),
        "logger_pending_queue": int(async_health.get("queue_size") or 0),
        "logger_last_successful_flush_age_ms": async_health.get("last_successful_write_age_ms", -1),
    }
    jp_accuracy_log("UI_PERFORMANCE_METRICS", **payload)
    if avg_insert > 15.0 or max_insert > 50.0:
        jp_accuracy_log("UI_TRANSCRIPT_INSERT_SLOW", **payload)
    if rss_mb > 0 and rss_mb > 1200:
        jp_accuracy_log("MEMORY_GROWTH_WARNING", **payload)
    flush_age = float(async_health.get("last_successful_write_age_ms") or -1)
    if flush_age > 10000:
        jp_accuracy_log("LOGGER_FLUSH_DELAY_WARNING", **payload)
        try:
            from alpha.utils.async_debug_log import log_runtime_debug_event

            log_runtime_debug_event("ASYNC_LOG_FLUSH_DELAY", **async_health)
        except Exception:
            pass
    try:
        from alpha.utils.async_debug_log import log_runtime_debug_event

        log_runtime_debug_event("ASYNC_LOG_QUEUE_DEPTH", queue_size=async_health.get("queue_size"))
    except Exception:
        pass
    return payload
