...
def get_accuracy_stage_compare_path(name: str, run_folder: str | Path | None = None) -> Path:
    mapping = {
        ...
        "raw_provider": "raw_provider.txt",
        "raw_deepgram_events": "raw_deepgram_events.jsonl",
        "raw_provider_events": "raw_provider_events.jsonl",
...
def record_raw_deepgram_final(
    *,
    ...
    try:
        ...
        # Language-agnostic alias for bilingual evidence capture.
        try:
            _append_jsonl(get_accuracy_stage_compare_path("raw_provider_events"), event)
        except Exception:
            pass
        with _lock:
...
def record_assembler_only_event(
    *,
    ...
    try:
        ...
        try:
            _append_jsonl(get_accuracy_stage_compare_path("stable_events"), event)
        except Exception:
            pass
        with _lock:
...
def evaluate_stage_capture_critical_checks(
    *,
    ...
    failed: list[str] = []
    lang = str(selected_language or "ja").lower()
    ...
    if is_english:
        # English Stable is evidence capture only (no Japanese ledger lineage).
        stable_events = count_assembler_event_total(run_folder)
        final_count = int(
            stable_final_compare.get("final_export_record_count")
            or stable_final_compare.get("final_text_line_count")
            or 0
        )
        _check("english_stable_event_count_nonzero", stable_events > 0)
        _check("english_final_record_count_nonzero", final_count > 0)
...
def build_stage_manifest(
    *,
    ...
    # Resolve actual Deepgram request language from snapshot/actual files.
    actual_deepgram_language = ""
    start_time = ""
    stop_time = ""
    duration_seconds = None
    for snap_name in ("deepgram_request_actual", "deepgram_request_snapshot"):
        snap = _load_json_if_exists(get_accuracy_stage_compare_path(snap_name, run_folder))
        if not snap:
            continue
        actual_deepgram_language = str(
            snap.get("language")
            or snap.get("actual_deepgram_language")
            or snap.get("selected_language")
            or actual_deepgram_language
            or ""
        )
        start_time = str(snap.get("start_time") or snap.get("created_at") or start_time or "")
    live_status = _load_json_if_exists(run_folder / "artifacts" / "LIVE_RUN_STATUS.json")
    if live_status:
        start_time = str(live_status.get("start_time") or live_status.get("started_at") or start_time or "")
        stop_time = str(live_status.get("stop_time") or live_status.get("stopped_at") or stop_time or "")
        if live_status.get("duration_seconds") is not None:
            duration_seconds = float(live_status.get("duration_seconds"))
    if duration_seconds is None and audio_summary:
        if audio_summary.get("run_elapsed_seconds") is not None:
            duration_seconds = float(audio_summary.get("run_elapsed_seconds"))
        elif audio_summary.get("captured_duration_seconds") is not None:
            duration_seconds = float(audio_summary.get("captured_duration_seconds"))

    stable_txt_info = _file_info("stable_transcript")
    if not stable_txt_info.get("exists"):
        stable_txt_info = stable_info
    final_record_count = int(
        stable_final_compare.get("final_export_record_count")
        or stable_final_compare.get("final_text_line_count")
        or final_info.get("line_count")
        or 0
    )
    raw_stage_complete = bool(raw_info.get("exists") and raw_events > 0)
    stable_stage_complete = bool(stable_info.get("exists") and asm_events > 0)
    final_stage_complete = bool(final_info.get("exists") and final_record_count > 0)
    final_authoritative_hash_match = bool(
        final_source_hash_matches
        or export_coverage.get("stage_final_hash_match")
        or export_coverage.get("sealed_final_hash_match")
    )
    audio_delivery_complete = bool(
        audio_info.get("exists")
        and audio_summary.get("generated_by_offline_repair") is not True
    )
    failure_codes = list(critical.get("stage_capture_failed_checks") or []) + list(missing)
    trusted_for_scoring = bool(
        complete
        and raw_stage_complete
        and stable_stage_complete
        and final_stage_complete
        and final_authoritative_hash_match
        and not repaired_offline
    )
    ...
    return {
        ...
        "run_id": run_id,
        "run_folder": str(run_folder).replace("\\", "/"),
        "run_type": run_type,
        "run_status": run_status,
        "selected_language": selected_language,
        "actual_deepgram_language": actual_deepgram_language or selected_language,
        "start_time": start_time,
        "stop_time": stop_time,
        "duration_seconds": duration_seconds,
        "raw_event_count": raw_events,
        "raw_text_char_count": int(raw_info.get("character_count") or 0),
        "stable_event_count": asm_events,
        "stable_text_char_count": int(
            stable_txt_info.get("character_count") or stable_info.get("character_count") or 0
        ),
        "final_record_count": final_record_count,
        "final_text_char_count": int(final_info.get("character_count") or 0),
        "raw_file_path": raw_info.get("path", ""),
        "stable_file_path": stable_info.get("path", ""),
        "final_file_path": final_info.get("path", ""),
        "raw_sha256": raw_info.get("sha256", ""),
        "stable_sha256": stable_info.get("sha256", ""),
        "final_sha256": final_info.get("sha256", ""),
        "raw_stage_complete": raw_stage_complete,
        "stable_stage_complete": stable_stage_complete,
        "final_stage_complete": final_stage_complete,
        "final_authoritative_hash_match": final_authoritative_hash_match,
        "audio_delivery_complete": audio_delivery_complete,
        "reference_visible_to_runtime": False,
        "offline_repair_used": bool(repaired_offline),
...
def finalize_accuracy_stage_artifacts(
    *,
    ...
    # Language-agnostic Raw provider aliases (raw_deepgram.txt → raw_provider.txt).
    try:
        raw_txt = get_accuracy_stage_compare_path("raw_deepgram", run_folder)
        raw_alias = get_accuracy_stage_compare_path("raw_provider", run_folder)
        if (not raw_txt.exists() or raw_txt.stat().st_size <= 0) and (
            get_accuracy_stage_compare_path("raw_deepgram_events", run_folder).exists()
        ):
            # Materialize TXT from events if missing.
            events_path = get_accuracy_stage_compare_path("raw_deepgram_events", run_folder)
            lines_out: list[str] = []
            for line in events_path.read_text(encoding="utf-8", errors="replace").splitlines():
                ...
                try:
                    obj = json.loads(line)
                    txt = str(obj.get("raw_text") or "").strip()
                    if txt:
        ...
        if raw_txt.exists() and raw_txt.stat().st_size > 0:
            raw_alias.write_text(raw_txt.read_text(encoding="utf-8"), encoding="utf-8")
            _jp_log("RAW_PROVIDER_ALIAS_WRITTEN", path=str(raw_alias))
    except Exception as exc:
        result["warnings"].append(f"raw_provider_alias:{exc}")
