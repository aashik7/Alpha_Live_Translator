...
def _strict_joined_text(text: str) -> str:
    """Strict scorer join: strip speaker tags / blanks, drop inter-token whitespace only."""
    lines: list[str] = []
    for raw in (text or "").splitlines():
        line = raw.strip()
        if not line or line.startswith("#"):
...
def _compact_prefix_len_to_strict_index(strict: str, compact_target: int) -> int:
    """Smallest strict index whose compact form length is >= compact_target."""
    if compact_target <= 0:
        return 0
    lo = 0
    hi = len(strict)
    while lo < hi:
        mid = (lo + hi) // 2
        if len(_norm_compact(strict[:mid])) < compact_target:
            lo = mid + 1
        else:
            hi = mid
    return lo
...
def _compact_prefix_len_to_strict_index_le(strict: str, compact_target: int) -> int:
    """Largest strict index whose compact form length is <= compact_target."""
    if compact_target < 0:
        return 0
    lo = 0
    hi = len(strict)
...
def _boundary_needles(sentence: str) -> list[str]:
    """Deterministic compact needles for semantic start/end location."""
    needles: list[str] = []
    for item in _candidate_needles(sentence):
        if item and item not in needles:
            needles.append(item)
    # Prefer longer needles first for stable boundary location.
    needles.sort(key=len, reverse=True)
    return needles


def clip_text_to_semantic_window(
    text: str,
    window: dict[str, Any],
    *,
    ...
    """Clip transcript text to the same first/last reference semantic boundary.

    Boundary-spanning records keep their in-window text instead of being discarded.
    Matching uses compact meaning-normalization; the returned slice preserves the
    strict scorer character set (including Japanese punctuation).
    """
    if not text or not window.get("window_resolved"):
        return text
    first = str(first_sentence or window.get("first_reference_sentence") or "")
    last = str(last_sentence or window.get("last_reference_sentence") or "")
    if not first or not last:
        return text

    strict = _strict_joined_text(text)
    if not strict:
        return text
    compact = _norm_compact(strict)
    if not compact:
        return text

    start_needles = _boundary_needles(first)
    # Prefer the leading clause of the first reference sentence when present.
    first_clause = re.split(r"[。！？!?]", first, maxsplit=1)[0].strip()
    if first_clause:
        clause_n = _norm_compact(first_clause)
        if clause_n:
            start_needles = [clause_n] + [n for n in start_needles if n != clause_n]
    # Window event-anchor needle is a fallback only (may sit mid-sentence).
    sa = str((window.get("start_anchor") or {}).get("needle_used") or "")
    if sa and sa not in start_needles:
        start_needles.append(sa)

    end_needles = _boundary_needles(last)
    last_clause_parts = [c.strip() for c in re.split(r"[。！？!?]", last) if c.strip()]
    if last_clause_parts:
        last_clause_n = _norm_compact(last_clause_parts[-1])
        if last_clause_n:
            end_needles = [last_clause_n] + [n for n in end_needles if n != last_clause_n]
    ea = str((window.get("end_anchor") or {}).get("needle_used") or "")
    if ea and ea not in end_needles:
        end_needles.append(ea)

    c_start: Optional[int] = None
    for needle in start_needles:
        if len(needle) < _MIN_NEEDLE and needle != _norm_compact(first):
            continue
    ...
    for needle in end_needles:
        if len(needle) < _MIN_NEEDLE and needle != _norm_compact(last):
            continue
        pos = compact.rfind(needle)
        if pos >= 0:
    ...
    s0 = _compact_prefix_len_to_strict_index_le(strict, c_start)
    s1 = _compact_prefix_len_to_strict_index(strict, c_end)
    if s1 <= s0:
        return text
    return strict[s0:s1]


def clip_record_text_to_semantic_window(
    record_text: str,
    window: dict[str, Any],
    *,
    first_sentence: Optional[str] = None,
    ...
    """Preserve in-window text for a single boundary-spanning record."""
    return clip_text_to_semantic_window(
        record_text,
        window,
