...
def build_audio_delivery_summary(
    *,
    ...
    crosscheck = verify_counter_crosscheck(host)
    missing: list[str] = []
    chunks_created = int(snap.get("mixed_audio_chunks_created") or 0)
    chunks_sent = int(snap.get("audio_chunks_sent") or 0)
    chunks_failed = int(snap.get("deepgram_send_errors") or 0) + int(snap.get("capture_errors") or 0)
    chunks_dropped = int(snap.get("audio_chunk_drop_count") or 0) + int(
        snap.get("audio_queue_overflow_count") or 0
    )
    chunks_queued = max(chunks_created, chunks_sent)
    chunks_pending_at_stop = max(0, chunks_queued - chunks_sent - chunks_dropped - chunks_failed)
    if chunks_created > 0:
        audio_delivery_ratio = round(float(chunks_sent) / float(chunks_created), 6)
    elif chunks_sent > 0:
        audio_delivery_ratio = 1.0
    else:
        audio_delivery_ratio = 0.0

    selected_language = ""
    actual_request_language = ""
    try:
        selected_language = str(
            getattr(host, "_listen_language", None)
            or getattr(host, "selected_language", None)
            or ""
        )
        actual_request_language = str(
            getattr(host, "_deepgram_request_language", None)
            or getattr(host, "_listen_language", None)
    ...
    payload = {
        ...
        "mixed_audio_chunks_created": chunks_created,
        "capture_errors": int(snap.get("capture_errors") or 0),
        "deepgram_send_errors": int(snap.get("deepgram_send_errors") or 0),
        # Benchmark-required aliases
        "chunks_created": chunks_created,
        "chunks_queued": chunks_queued,
        "chunks_sent": chunks_sent,
        "chunks_failed": chunks_failed,
        "chunks_dropped": chunks_dropped,
        "chunks_pending_at_stop": chunks_pending_at_stop,
        "audio_delivery_ratio": audio_delivery_ratio,
        "captured_duration_seconds": elapsed,
        "sent_duration_seconds": calc_seconds,
        "first_audio_timestamp": snap.get("first_audio_timestamp"),
        "last_audio_timestamp": snap.get("last_audio_timestamp"),
        "selected_language": selected_language,
        "actual_request_language": actual_request_language,
