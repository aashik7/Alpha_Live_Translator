...
APP_VERSION = "3.3.5.5.8.5.26.5.14"
...
# Bound actively rendered Tk transcript history (canonical store remains complete).
...
UTTERANCE_COMMIT_FALLBACK_MS = 2000
UTTERANCE_LIFECYCLE_ENABLED = True
...
# Manual dropdown is authoritative. Do not force-lock Deepgram to ja —
# that overrode English selection (English → ja regression).
# Optional diagnostic override only; live routing must leave this None/empty.
...
# English Deepgram diarization — disabled (no diarize / diarize_model params).
# Japanese STT profile remains independent (JAPANESE_STT_PROFILE=no_diarize).
ENGLISH_DIARIZATION_ENABLED = False
# UI-only speaker prefix (not part of Raw/Stable/Final lexical scoring).
UI_SPEAKER_LABEL = "Speaker:"

# Async DeepL translation (Stable segments only)
