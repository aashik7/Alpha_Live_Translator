...
| 44 | Deepgram websocket reconnect mid-session: backoff, buffer, commit in-flight, mark the gap visibly | **Opus 5** | 7 | **DONE 2026-08-12 (code); live gate still open.** Backoff, replay buffer and single-flight reconnect were **already shipped** ("fix 5") — verified, not rebuilt. The missing piece was **marking the gap**: audio captured while the socket is down is gone, and replay cannot bring it back, so the transcript silently stitched across the hole. `_mark_deepgram_gap_if_any` now emits a visible marker with the real outage length, flagged `synthetic_record` + `translation_eligible: False`. Clock starts only on an **unexpected** close, so a normal Stop is never annotated. Tests `tests/test_resilience_gap_and_circuit.py`. **The sprint's own gate — a 60-min session with a deliberate drop — needs a human and is NOT signed off** |
