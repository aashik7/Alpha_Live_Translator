...
| 66 | English commits land mid-sentence: 5 of 7 records on run `...161651` end without a terminator | **Opus 5** | — | **OPEN.** `utterance_end` / `inactivity_timeout_fallback` commit whatever has accumulated, mid-sentence or not, so one spoken sentence is split across two canonical records. Formatting cannot repair it — by then the text is in two records. Fixing it means changing English commit timing, the same area where item 65's flush lost 8 of 9 utterances, so it needs that loss understood first |
