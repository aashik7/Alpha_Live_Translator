...
**Baseline:** **410 tests** (was 354; item 38 added 10, item 38b added 4,
item 39 added 17, item 51 added 25), 5 failures + 2 errors + 2 skipped.
Same 7 names. Any change to that set means you broke something.
...
| 41 | Prove problem A's root cause against run `...20260807-160529` — prove, do not assume | **Opus 5** (actually used) | 3–4 | **INVESTIGATED — root cause strongly indicated, NOT proven. Stop-and-report per item 41 §10.** Proof document: `PROBLEM_A_ROOT_CAUSE.md` (repo root — `troubleshooting/` is gitignored). Established: true loss count is **10, not 14** (item 38's detector over-reports); loss occurs **iff** a `canonical_utterance_id` carries >1 stable row with *disjoint* text; `decide_stable_revision_action` returned `append` in **every** case while the id-mint gate reused the id, separating 9/9 (loss) vs 3/3 (no loss) on `update_previous_requested`. **Phase 5 failed — no deterministic reproduction**, so item 42 cannot yet satisfy §4 step 5. See §9 |
