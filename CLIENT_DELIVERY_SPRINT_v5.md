...
| 44 | Deepgram websocket reconnect mid-session: backoff, buffer, commit in-flight, mark the gap visibly | **Opus 5** | 7 | **THIRD FIX 2026-08-12, still needs a live retest.** Keepalive (fix 2) worked — run `...142447` detected the dead socket and marked a gap. It still never recovered, for a different reason: **reconnect made exactly ONE attempt.** `_schedule_reconnect` is single-flight on `_dg_reconnecting`, and websocket-client calls `on_close` from *inside* `run_forever`, so the `on_close -> _schedule_reconnect()` retry signal arrived while the flag was still set and was dropped; the `finally` then cleared it with nobody left to call. Retrying is now the loop's own job (backoff 1s doubling to 30s, per-attempt exception handling, 1s floor so a 0 backoff cannot spin). Gap marker moved to `_deepgram_on_open` — it reported `2.6s` for a 45s outage because it fired before the socket was known to connect. Tests `tests/test_reconnect_retries_and_translation_backoff.py` |
