...
| | Problem | Evidence |
|---|---|---|
| **F** | **English commits self-concatenate on every reformatting variant Deepgram sends for the same growing utterance** (e.g. "50 percent" then "50%", "mister" then "Mr."), instead of replacing — corruption compounds every interim tick and lands in the canonical ledger, translation input, and final export | Root-caused and reproduced against the real function 2026-08-11: `utterance_lifecycle.py` `_merge_lexical()` (~L91) falls through to its adjacent-chunk concatenation branch (~L126-131) whenever two variants aren't a literal substring match; `_apply_active_update_locked()` (~L1544) then writes that concatenated `merged` to `active.text` unconditionally, even on its own `REPLACE_ACTIVE` branch. On the live English test run (`...182940`): 5 of 54 export lines carry **86% of the export's characters** by volume, one line 5039 characters from ~112 comma-joined fragments. Confirmed pre-existing, not new: run `...133236` (2026-08-08) shows the identical pattern. Japanese is unaffected — confirmed clean both by direct reading and because `should_use_utterance_lifecycle()` routes Japanese through a different function entirely. See §9 |
...
**Baseline:** **385 tests** (was 354; item 38 added 10, item 38b added 4,
item 39 added 17), 5 failures + 2 errors + 2 skipped. Same 7 names. Any
change to that set means you broke something.
...
| 51 | Fix problem F: English commits self-concatenate on reformatting variants (`utterance_lifecycle.py` `_merge_lexical`/`_apply_active_update_locked`) | **Opus 5** | — | **Root-caused, not fixed — recommend re-prioritizing ahead of 41-43, human call.** `[gate]`: the exact boundary between "reformatted revision of the same utterance" and "genuinely new adjacent chunk" is a design decision, and this is a shared function on every English commit. See §1 row F, §9 |
