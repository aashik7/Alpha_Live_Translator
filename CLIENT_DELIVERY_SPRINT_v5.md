...
| 44 | Deepgram websocket reconnect mid-session: backoff, buffer, commit in-flight, mark the gap visibly | **Opus 5** | 7 | **REOPENED then FIXED 2026-08-12.** My earlier claim that reconnect "already worked (fix 5)" was **wrong** — I verified the code existed, never that it ran. A live network-drop test proved it never runs: `run_forever()` had no `ping_interval`, so a WiFi drop (no FIN) left it blocked forever, `on_close` never fired, and the entire reconnect chain was unreachable in the one scenario it exists for. Run `...20260812-095935`: **411 audio chunks discarded, zero reconnect events, transcript frozen, `final_status: failed`**. Added `DG_WS_PING_INTERVAL_S=10` / `DG_WS_PING_TIMEOUT_S=5`. Tests `tests/test_deepgram_keepalive.py`. **Still needs a live retest to confirm** — 5 minutes is enough |
| 45 | DeepL failure/quota: degrade visibly, retry with backoff, circuit-break after N, never block the transcript | **Opus 5** | 8 | **DONE 2026-08-12.** Retry-with-backoff and error classification were **already shipped** — verified, not rebuilt. Added the **circuit breaker**: opens after `TRANSLATION_CIRCUIT_BREAK_AFTER` (3) *consecutive* failures, cooldown 30s doubling to a 300s cap, always expires so recovery is never refused, and one success closes it. `degraded` property exposes it for item 47. Quota is deliberately excluded (it already disables permanently). Translation is on a worker thread, so a provider outage never blocks a transcript commit |
...
| 65 | English has no sentence boundary: one exported line held 25 sentences / 2445 chars | **Opus 5** | — | **DONE 2026-08-12.** Japanese gets boundaries from the assembler; English relies on Deepgram `speech_final`, which continuous speech never sends. Config was correct (endpointing=1200, utterance_end_ms=1500) — the speaker just never paused 1.2s, so one utterance took 45s across 187 revisions. New `_flush_sentence_boundary_locked` commits at a finished sentence, triggered ONLY when the merge was exactly `prev + " " + curr` (the `.!?` concatenation branch), so no revision path can be split. Commits and publishes rather than dropping; returns False if the commit is refused. Reason deliberately NOT in `_PREMATURE_COMMIT_REASONS` |
