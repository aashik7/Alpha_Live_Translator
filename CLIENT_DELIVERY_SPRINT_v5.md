...
| 44 | Deepgram websocket reconnect mid-session: backoff, buffer, commit in-flight, mark the gap visibly | **Opus 5** | 7 | **FOURTH FIX 2026-08-12.** Run `...150116` proves fixes 2 and 3 work: the drop was detected, reconnect retried **5 times** with 1/2/4/8/16s backoff, `DEEPGRAM_CONNECT_END` fired, and the gap marker read **30.4s for a 34s outage**. The keepalive then crashed the app. `ping_interval` starts websocket-client's `_send_ping` thread, whose 1.6.0 body reads `self.stop_ping.wait(...)` unguarded; it raced with teardown during the drop and raised `AttributeError: 'NoneType' object has no attribute 'wait'` in a bare thread — `CRASH_HOOK_TRIGGERED`, `UNHANDLED_EXCEPTION_CAPTURED`, run recorded `crashed` at 15:03:50. Self-inflicted: there was no ping thread before the keepalive. Fixed with `_keepalive_websocket_app_class()`, a `WebSocketApp` subclass that swallows the AttributeError **only** when `stop_ping` is actually gone. Tests `tests/test_keepalive_ping_thread_cannot_crash.py` |
