...
| 65 | English has no sentence boundary: unreadable wall-of-text lines | **Opus 5** | — | **FIXED 2026-08-12 (third approach).** Solved as *formatting*, not as commit timing. New `alpha/utils/english_line_grouping.py` regroups an utterance into 2-3 sentence lines: always break at 3, break early at 2 once the line reaches a 20-word budget — the user's rule ("long: break after 2; short: break after 3; I cannot count letters, use a word limit"). His own example is the spec and is reproduced exactly. Wired into `TranscriptStore.get_clean_text`, the single place the readable transcript is built, so UI and copy/export both get it. English only; Japanese keeps the assembler's boundaries. Real run `...154956`: one 2342-char / 27-sentence line becomes 13 lines, longest 355 chars, **every word preserved** (asserted). Fails closed to the original text if preservation ever fails. Attempt two's lifecycle flush stays OFF |
