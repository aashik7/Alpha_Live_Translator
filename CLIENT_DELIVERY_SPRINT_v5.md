...
**Baseline:** **405 tests** (was 354; item 38 added 10, item 38b added 4,
item 39 added 17, item 51 added 20), 5 failures + 2 errors + 2 skipped.
...
| 51 | Fix problem F: English commits self-concatenate on reformatting variants and slid windows (`utterance_lifecycle.py` `_merge_lexical`) | **Opus 5** | — | **DONE 2026-08-11**, gate approved by the human before implementation. Three changes, all inside `_merge_lexical`: compare tokens with edge punctuation stripped; try an exact boundary-run join *before* the fuzzy similarity gate; measure the order gate over the shorter side (the 0.6 thresholds themselves are unchanged). Tests `test_utterance_lifecycle_merge_lexical.py` (20), proven to fail 8/8 on pre-fix code. **Still needs one live English re-run to confirm export-wide** — see §9 |
