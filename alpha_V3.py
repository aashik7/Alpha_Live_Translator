"""
Alpha V3 - Live Meeting Translation Desktop Application
Dual audio capture (WASAPI loopback + gated microphone), speaker-colored text,
and Deepgram real-time STT.
"""
...
MIC_NOISE_GATE_RMS = 30  # mic RMS below this is treated as silence (echo prevention)
MAX_AUDIO_QUEUE_SIZE = 60
...
def _apply_noise_gate(audio_np, threshold=MIC_NOISE_GATE_RMS):
...
class AlphaApp(ctk.CTk):
    """Main application window for Alpha live meeting translation (V3)."""
    ...
    def __init__(self):
        ...
        self.last_transcript_hash = None
    ...
    def audio_mixer_worker(self):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                # Process system audio (always send it)
                sys_np = np.frombuffer(sys_chunk, dtype=np.int16)

                # Stereo to mono if needed
                if self._wasapi_channels > 1 and len(sys_np) >= self._wasapi_channels:
                    frame_count = len(sys_np) // self._wasapi_channels
                    sys_np = sys_np[: frame_count * self._wasapi_channels]
                    sys_np = sys_np.reshape(-1, self._wasapi_channels).mean(axis=1).astype(np.int16)

                # Resample 48kHz → 16kHz (or generic integer ratio)
                if self._wasapi_rate > DEEPGRAM_SAMPLE_RATE and self._wasapi_rate % DEEPGRAM_SAMPLE_RATE == 0:
                    sys_np_16k = sys_np[:: self._wasapi_rate // DEEPGRAM_SAMPLE_RATE]
                else:
                    sys_np_16k = _pcm_to_mono_16k_np(
                ...
                # Only mix mic if we have it AND it passes noise gate
                if mic_chunk is not None:
                    ...
                    if mic_np.size > 0:
                        ...
                        # Only mix if mic has significant audio
                        if rms > 100:
                            # Align lengths
                            ...
                            if min_len > 0:
                                ...
                                # Weighted mix to prioritize system speakers
                                mixed = (
                                    sys_aligned.astype(np.float32) * 0.7
                                    + mic_aligned.astype(np.float32) * 0.3
                                )
                                mixed = np.clip(mixed, -32768, 32767).astype(np.int16)
                                final_chunk = mixed.tobytes()
                                sys_rms = float(np.sqrt(np.mean(sys_aligned.astype(np.float32) ** 2)))
                                print(
                                    f"[MIX] Mixed chunk: sys_rms={sys_rms:.1f}, "
                                    f"mic_rms={rms:.1f} -> {len(final_chunk)} bytes"
                                )
                            else:
                                final_chunk = sys_np_16k.tobytes()
                        else:
                            # Mic too quiet, just use system audio
                            final_chunk = sys_np_16k.tobytes()
                    else:
                        final_chunk = sys_np_16k.tobytes()
                else:
                    # No mic audio, just use system
                    final_chunk = sys_np_16k.tobytes()
                ...
                # No audio for 100ms - normal during silence
                pass
    ...
    def extract_speaker_and_text(self, data):
        """Extract speaker number and transcript from Deepgram response."""
        transcript = ""
        try:
            ...
            # Prefer the dominant speaker in this utterance instead of first word only.
            speaker_votes = {}
            for word in words:
            ...
            if speaker_votes:
                ...
                return dominant_speaker + 1, transcript  # Deepgram is 0-indexed

            if "speaker" in data:
                return data["speaker"] + 1, transcript

            print("Warning: No speaker data found in Deepgram response!")
            return 1, transcript
        except Exception as e:
            print(f"Error extracting speaker: {e}")
            return 1, transcript

    def extract_speaker_simple(self, data):
        """Extract speaker id from Deepgram diarization with multiple fallbacks."""
        try:
            alternatives = data.get("channel", {}).get("alternatives", [])
            if not alternatives:
                print("[Diarization] source=none speaker=1")
                return 1
            ...
            for word in words:
                ...
                if raw_speaker is None:
                    continue
                sid = int(raw_speaker) + 1  # Deepgram is 0-indexed
                speaker_votes[sid] = speaker_votes.get(sid, 0) + 1

            if speaker_votes:
                speaker = max(speaker_votes.items(), key=lambda x: x[1])[0]
                print(
                    f"[Diarization] source=words speaker={speaker} "
                    f"votes={speaker_votes}"
                )
                return speaker

            paragraphs = alt.get("paragraphs", {}) or {}
            para_list = paragraphs.get("paragraphs", []) or []
            if para_list:
                para_speaker = para_list[0].get("speaker")
                if para_speaker is not None:
                    speaker = int(para_speaker) + 1
                    print(f"[Diarization] source=paragraph speaker={speaker}")
                    return speaker

            utterances = data.get("utterances", []) or []
            if utterances:
                utt_speaker = utterances[0].get("speaker")
                if utt_speaker is not None:
                    speaker = int(utt_speaker) + 1
                    print(f"[Diarization] source=utterance speaker={speaker}")
                    return speaker

            if data.get("speaker") is not None:
                speaker = int(data["speaker"]) + 1
                print(f"[Diarization] source=top_level speaker={speaker}")
                return speaker

            print("[Diarization] source=default speaker=1")
            return 1
        except Exception as e:
            print(f"Error in extract_speaker_simple: {e}")
            return 1
    ...
    def _deepgram_on_message(self, _ws, message):
        ...
        try:
            ...
            speaker_num = self.extract_speaker_simple(data)
            ...
            print(f"[Transcript] Speaker {speaker_num}: {transcript}")
        except Exception as exc:
    ...
    def process_ui_queue(self):
        ...
        try:
            while not self.ui_queue.empty():
                ...
                if item.get("kind") == "interim":
                    continue
                speaker_id = item.get("speaker", 1)
                text = item.get("text", "").strip()
                if not text:
                    continue

                current_hash = hash(f"{speaker_id}:{text}")
                if current_hash == self.last_transcript_hash:
                    continue
                self.last_transcript_hash = current_hash
                ...
                if (
                    self.last_speaker_id is not None
                    and speaker_id != self.last_speaker_id
                ):
                ...
                tag = f"speaker_{speaker_id}"
                if tag not in self.initial_verse_box.tag_names():
                    tag = "body"
                self.initial_verse_box.tag_add(tag, start_idx, end_idx)
    ...
    def _clear_interim_preview(self, box):
        """Remove the live interim preview line without touching committed text."""
        if "interim_live" in box.mark_names():
            box.delete("interim_live", tk.END)

    def _handle_interim_transcript(self, item):
        """Show a single live interim preview line (replaced on each update)."""
        ...
        self._clear_interim_preview(box)
    ...
    def _handle_final_transcript(self, item):
        """Append a final transcript line with duplicate and speaker-change handling."""
        box = self.initial_verse_box
        if box is None:
            return

        speaker_num = item.get("speaker", 1)
        transcript_text = item.get("text", "").strip()
        if not transcript_text:
            return

        current_hash = hash(f"{speaker_num}:{transcript_text}")
        if current_hash == self.last_transcript_hash:
            return
        self.last_transcript_hash = current_hash
        ...
        # ==========================================
        # TODO: DEEPL TRANSLATION INTEGRATION
        # When DeepL is added, send 'transcript_text'
        # to the DeepL API here and put the result
        # into the 'Translated verse' text box.
        # ==========================================
        ...
        if self.last_speaker is not None and speaker_num != self.last_speaker:
            box.insert(tk.END, "\n\n")

        speaker_label = f"[Speaker {speaker_num}]"
        speaker_tag = f"speaker_{speaker_num}" if 1 <= speaker_num <= 4 else "body"
        start_idx = box.index(tk.END)
        box.insert(tk.END, speaker_label + " ")
        end_idx = box.index(tk.END)
        box.tag_add(speaker_tag, start_idx, end_idx)
        box.insert(tk.END, transcript_text + "\n", "body")
