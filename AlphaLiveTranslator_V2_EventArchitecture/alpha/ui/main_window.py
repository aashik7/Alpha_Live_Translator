"""
Alpha Live Translator — main window (V2 Event Architecture).
...
class AlphaApp(
    ...
    def publish_translation_event(
        ...
        """Publish translation placeholder event (DeepL integration pending)."""
    ...
    def _on_app_status_changed(self, payload: StatusEvent):
        ...
        def update():
            if self.status_text_label is not None and payload.message:
                prefix = "🎤 " if payload.status == "listening" else "🎤 "
                self.status_text_label.configure(text=f"{prefix}{payload.message}")
    ...
    def _on_translation_received(self, payload: TranslationEvent):
        """Translation placeholder — UI box update deferred to V3."""
        if not isinstance(payload, TranslationEvent):
            return
        logger.debug("Translation event received: %s", payload.translated_text[:80])
        # TODO V3: Move translated_verse_box updates to this handler.
