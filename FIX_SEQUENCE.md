...
| owner request, with item 30 | **Item 31** — a meeting that runs but does not work names itself in the status strip, in the display language, and a click shows the whole sentence: no sound for a minute, sound with nothing transcribed for 30 s (thresholds measured on the retained runs), a switched-on microphone failing, no DeepL key, DeepL quota used up or key rejected. Also: DeepL errors told apart by type and status, not by a digit in the text | ✅ fixed, package 26.5.29 |
| found reviewing item 30 | **Item 32** — the `.env` beside the app outranks a stale `DEEPGRAM_API_KEY` or `DEEPL_AUTH_KEY` in the machine's environment, so editing the file the app tells you to edit actually changes the key it uses. A name the file does not carry still comes from the environment | ✅ fixed, package 26.5.30 |
| owner request, the other half of the item 28 report | **Item 33** — the meeting's language is fixed for the run: both header combos, both hamburger combos and the swap button lock from “Starting…” and come back as pickers on Stop. Deepgram is told the language when the socket opens and cannot be told again, so the switch never worked mid-meeting. Two leaks closed with it: the whole-combo click bypassed every state check, and a list already posted when the lock landed still delivered its pick. The UI's own display language stays switchable | ✅ fixed, 26.5.31 (package not built yet) |
| owner request, from the item 32 report | **Item 34** — a keyterm rejection at Start retries once without them instead of ending the Start. The fallback existed but was unreachable there: `stop_requested` counts `not is_listening` as a stop, and that is what Start looks like. One decision function now answers it for both Start and mid-meeting, and `_deepgram_worker` reopens the socket once, because `_schedule_reconnect` refuses while not listening | ✅ fixed, 26.5.32 (package not built yet) |
| owner's session 2026-09-25 16:40:43 | **Item 35** — one utterance is one line: a revision Deepgram sends after the app committed its early guess (same first-word start) re-opens that record and supersedes it, instead of becoming a new line; two guesses at the same audio are never glued. And English revisions now reach the ledger at all -- two gates (the registry's record id read as "already written", then `idempotent_replay`) had kept every one of them, including the existing final-correction and extend paths, out of the export | ✅ fixed, 26.5.33 (package not built yet) |
| owner's end-to-end test 2026-09-25 | **Item 36** — Deepgram is no longer asked for `numerals`, which on English wrote "third quarter" as "3rd 0.25" and "a quarter of the team" as "0.25 of the team" (measured live; DeepL carried it into the translation). `smart_format` still writes 12%, dates and times; Japanese output was byte-identical with and without it | ✅ fixed, 26.5.34 (package not built yet) |
